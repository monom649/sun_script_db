...
def handler(event, context):
    ...
    if event['httpMethod'] == 'GET':
        try:
            # URLパラメータを取得
            query_params = parse_qs(event.get('queryStringParameters', {}).get('q', '') if event.get('queryStringParameters') else '')
            keyword = query_params.get('q', [''])[0] if query_params else event.get('queryStringParameters', {}).get('q', '') if event.get('queryStringParameters') else ''
